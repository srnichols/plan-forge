...
- **`OTEL_ENABLED=true` without `OTEL_EXPORTER_OTLP_ENDPOINT` silently drops spans.** The OTLP exporter falls back to `localhost:4318`, so with no collector listening the spans go nowhere. Measured: this does **not** crash a running server — a process emitting spans with no collector survives indefinitely, because the background exporter swallows the connection failure. The uncaught `AggregateError [ECONNREFUSED]` is reachable only via an explicit `sdk.shutdown()`, which Plan Forge never calls. Worth knowing before adding graceful shutdown, and worth documenting so `OTEL_ENABLED` is not mistaken for a standalone switch.
