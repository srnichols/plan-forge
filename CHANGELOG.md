...
- **`autocannon` moderate advisories: no action.** npm proposes `autocannon@2.0.1` as the "fix", which is a **downgrade** from the current 8.0.0. The vulnerability is a transitive `hyperid` → `uuid` moderate in a load-testing tool that is lazily imported only when the load-stress scanner runs. Downgrading six majors of a runtime dependency is a worse trade than the moderate. These are the **only** advisories left — zero critical, zero high.
