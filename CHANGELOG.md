...
- **OpenTelemetry tracing was silently inert whenever it was switched on.** `initOtel()` swallowed the dynamic-import error and returned `null`, so `OTEL_ENABLED=true` produced no traces and no diagnostic. That masked a packaging bug: `sdk-node` requires `@opentelemetry/api` as a **peer** (`>=1.3.0 <1.10.0`), Plan Forge declared it only in `optionalDependencies`, and npm silently drops it — no lockfile entry, no error. The catch now warns with the underlying error and the exact install command; the `null` return and graceful-degradation contract are unchanged. **The packaging fix itself is still open** — see Known Issues.
...
- **`@opentelemetry/api` cannot be installed via `optionalDependencies`.** npm silently drops it (no lockfile entry, no error), and `sdk-node` needs it as a required peer, so tracing cannot start. Needs a packaging decision: promote the OTel packages to real `dependencies`, or remove them entirely and document a user-run install command. `initOtel()` now warns instead of failing silently in the meantime.
- **`vitest` 2.1.9 → 4.1.10 deferred.** Fixes the last critical advisory, but it is dev-only (never shipped). Measured: **36 tests across 11 files fail** on v4 — 20 in `notifications-stubs.test.mjs` alone, plus scattered failures in `tempering-integration`, `quorum-probe`, `cost-service` parity, and routing tests. Likely 3–5 root causes rather than 36, but the parity and probe failures could reflect genuine mock-semantics changes, so this needs a focused migration rather than batch test edits. Note `--reporter=basic` was removed in v3+ and will need replacing in any gates that use it.
