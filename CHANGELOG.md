...
- **Bash setup accepts valid multi-preset selections** ([#277](https://github.com/srnichols/plan-forge/issues/277)). Preset lists are validated and split before the display label is chosen from the primary preset, so `--preset typescript,dotnet` reaches the existing combined installation path.
...
- **Tempering adapters ship inside the runtime package** ([#269](https://github.com/srnichols/plan-forge/issues/269), [#271](https://github.com/srnichols/plan-forge/issues/271)). All nine stack adapters now live under `pforge-mcp/tempering/adapters/`, so both setup and self-update deliver them without preset-file collisions. Mutation scanning uses the same shipped location, asynchronous adapter loads are awaited, and fully skipped mandatory test scanners cannot produce a passing verdict.
- **Plan parsing and hardening reject misleading success** ([#251](https://github.com/srnichols/plan-forge/issues/251), [#255](https://github.com/srnichols/plan-forge/issues/255), [#256](https://github.com/srnichols/plan-forge/issues/256), [#257](https://github.com/srnichols/plan-forge/issues/257), [#258](https://github.com/srnichols/plan-forge/issues/258), [#260](https://github.com/srnichols/plan-forge/issues/260)). Prose and illustrative fences no longer become scope paths or executable gates. Letter-suffixed slices receive lock-hash coverage, zero parsed slices fail lint, status-heading variants are recognized, and hardening checks require discoverable memory evidence and gates capable of failing.
- **Consumer application versions remain separate from Plan Forge's version** ([#250](https://github.com/srnichols/plan-forge/issues/250), [#253](https://github.com/srnichols/plan-forge/issues/253)). Update checks resolve the installed framework version from its own manifest, and Smith only compares the root VERSION namespace inside Plan Forge itself. Smith also reads VS Code settings as JSONC in both shells ([#252](https://github.com/srnichols/plan-forge/issues/252)).
- **Memory captures avoid nested envelopes and recover existing nested records** ([#254](https://github.com/srnichols/plan-forge/issues/254)). Run pruning now honors age as well as count and compacts the run index ([#259](https://github.com/srnichols/plan-forge/issues/259)); the legacy synchronous `pruneRunHistory` export remains available as a deprecated count-only wrapper for compatibility.
- **BYOK SDK routing retains the environment-variable name needed for key lookup**, with regression coverage for the routing branches ([#267](https://github.com/srnichols/plan-forge/issues/267), [#268](https://github.com/srnichols/plan-forge/issues/268)). Keys remain resolved at call time rather than embedded in provider configuration.
- **Coverage layers support operator-defined `layerGlobs`** ([#270](https://github.com/srnichols/plan-forge/issues/270)). Unclassified files are reported, and configured layer minima are not treated as passing measurements when no files were classified into those layers.
...
## [3.26.5] — 2026-08-10 — Consumer test suite runs clean after self-update
...
- **`npm test` went red on a healthy consumer install** ([#249](https://github.com/srnichols/plan-forge/issues/249)). `pforge self-update` copies `pforge-mcp/` recursively, so consumers receive Plan Forge's entire internal test suite — but not the source-repo tooling those suites exercise (`scripts/audit/`, `scripts/forge-home-cleanup.mjs`, `pforge-master/`, `pforge-sdk/`, `docs/manual/`, `docs/capabilities.md`, `docs/plans/testbed-scenarios/`, `templates/`, `presets/`). A clean v3.24.1 → v3.26.4 update passed `pforge check` 33/33 and Smith 63/63, then failed 174 tests across 27 files. `changelog-format.test.mjs` was worse than a missing fixture: it validates Plan Forge's own CHANGELOG against the repo-root `VERSION`, and in a consuming project both files exist but describe the *consumer's* application — so it compared their app version (3.38.1) against the MCP version and failed on a correct install. `vitest.config.mjs` now maps each source-only asset to the suites that need it and excludes a suite only when its asset is genuinely absent, so consumers run the runtime coverage that applies to them and the source repo is unaffected. Deliberately existence-driven rather than a hardcoded skip list: in this repo every asset is present, so **zero** suites are excluded, and `tests/consumer-test-surface.test.mjs` fails the build if that ever stops being true — a typo'd asset path would otherwise read as "missing" here and silently drop real coverage, which is the same drift that let the Smith Node-floor check assert `>= 18` long after the floor had moved.
...
## [3.26.3] — 2026-08-10 — Setup blocks unsupported Node runtimes
...
## [3.26.1] — 2026-08-10 — Copilot SDK default reverted to opt-in
...
- **`npm audit` now reports 0 vulnerabilities** (from 72 at the start of this cycle, 1 critical / 19 high). The last three moderates came from `autocannon` ΓåÆ `hyperid` ΓåÆ `uuid`, and are now gone because **`autocannon` is no longer bundled** ΓÇö see Changed. Worth recording that those three were never reachable: the advisory ([GHSA-w5hq-g745-h8pq](https://github.com/advisories/GHSA-w5hq-g745-h8pq)) is a missing buffer bounds check in uuid **v3/v5/v6 when `buf` is provided**, while `hyperid` calls `crypto.randomUUID` on modern Node and falls back to `uuid.v4` otherwise ΓÇö neither path reaches the vulnerable functions. `npm audit fix` proposed `autocannon@2.0.1`, a **six-major downgrade**, because 8.0.0 is already the latest and no forward fix exists.
- **`vitest` 2.1.9 ΓåÆ 4.1.10** ([#239](https://github.com/srnichols/plan-forge/issues/239)) clears the last **critical** advisory (`vite`, via vitest) plus the remaining high. The 36 measured failures resolved to three causes, not thirty-six: variable-specifier dynamic imports (vitest 4 resolves a non-literal specifier against the project root rather than the importing module ΓÇö 26 tests, including a latent fragility in the production `loadAdapter`), a `testTimeout` default too small for a suite that shells out to real CLI probes, and two contention-only perf budgets. **No test assertion was weakened** ΓÇö the `cost-service`/`estimate-slice` parity and `quorum-probe` availability tests flagged as masking risks all pass on their original assertions.
- **`sharp` and the OpenTelemetry SDK upgraded off high-severity advisories.** `sharp` 0.34.5 ΓåÆ 0.35.3 (native module ΓÇö all five conversion formats `jpg`/`png`/`webp`/`avif`/`gif` were exercised directly against libvips 8.18.3, since a green unit suite does not prove a native rebuild works). `@opentelemetry/sdk-node` 0.57.2 ΓåÆ 0.221.0 and `auto-instrumentations-node` 0.56.1 ΓåÆ 0.79.0.
- **`ws` high-severity advisories patched.** `ws` was pinned at 8.20.0 ΓÇö inside the vulnerable `8.0.0 - 8.20.1` range for [GHSA-58qx-3vcg-4xpx](https://github.com/advisories/GHSA-58qx-3vcg-4xpx) (uninitialized memory disclosure) and [GHSA-96hv-2xvq-fx4p](https://github.com/advisories/GHSA-96hv-2xvq-fx4p) (memory-exhaustion DoS from tiny fragments). It is a direct dependency of both `pforge-mcp` and `pforge-master` and backs the dashboard WebSocket, so it was reachable in a server users actually run. Now 8.21.3, along with non-breaking fixes for 13 transitive highs. Lockfile only ΓÇö no `package.json` ranges changed.
- **Command-injection surface removed from GitHub issue filing.** The `gh issue create` shell string interpolated agent-supplied title/body with only `"` escaped, leaving `$(...)`, backticks, and `$VAR` live on POSIX shells. The `gh issue list` dedupe lookup had the same shape, with `owner`/`repo` reaching it from `.forge.json#bugRegistry.githubRepo`. Both now pass an argv array ΓÇö no shell, no escaping, no injection surface (`security.instructions.md` Rule 1).

### Fixed

- **Crucible finalize claimed `Phase-01` in a repository already at Phase-59** ([#244](https://github.com/srnichols/plan-forge/issues/244)). `PHASE_NAME_RE` matches only a bare `Phase-NN`, and `collectExistingPhaseNames()` filtered plan basenames through it ΓÇö so every plan using this project's own `Phase-NN-NAME-PLAN.md` convention was discarded, the list came back empty, and `nextPhaseNumber([])` returned `Phase-01`. Not a one-off: each subsequent smelt would keep claiming low numbers while real history sat in the fifties. Fixed at the collection layer with `phaseNameFromPlanFile()`, which recovers the canonical `Phase-NN` from a suffixed filename; `isValidPhaseName`, `nextPhaseNumber` and `claimPhaseNumber` keep their existing semantics, so the test asserting `Phase-CRUCIBLE-01` is *not* a numbered phase stays correct and untouched. The repo now reports 14 numbered phases where it previously reported none. Found by using the tool: the smelt for Phase-60 was written as `Phase-01.md`, and the hand-picked replacement number was itself wrong because it was eyeballed from a truncated listing ΓÇö precisely the error the automated claim exists to prevent.
- **Crucible tool surface: unbounded payload, unconfirmed lane, malformed frontmatter** ([#245](https://github.com/srnichols/plan-forge/issues/245)). Three defects found while running a 13-question interview. `forge_crucible_ask` echoed the entire draft on every call, and the draft grows with each answer ΓÇö measured at 9 KB rising to 24 KB across one interview, against an ACI budget of roughly 10 KB, and redundant because `forge_crucible_preview` already exists for on-demand review. The draft is now opt-in via `includeDraft`, with a `draftHint` naming the preview tool; the dashboard's REST route opts in so its live-preview pane is unaffected. A default response is now ~357 bytes regardless of interview length. `forge_crucible_submit` echoed only `recommendedLane`, so submitting with `mode:"full"` still reported `"tweak"` and an agent could not tell whether its override applied ΓÇö it now returns `lane` (applied) alongside `recommendedLane` (heuristic). And the `linked-bugs` answer was split on commas and interpolated into a YAML flow sequence, so a prose reply emitted `linkedBugs: [free text with colons and #241 ΓÇª]`; inside a flow sequence an unquoted ` #` opens a comment. Tokens are now kept only when they are tracker-shaped (`RMG-0035`) ΓÇö the first attempt at this filter was too permissive and matched ordinary words like "research", which the tests caught.
- **`pforge` self-test reported 4 false failures and exited 1.** `node pforge-mcp/orchestrator.mjs --test` finished **105 passed, 4 failed** ΓÇö a shipped diagnostic telling every user the tool is broken. Two independent causes. Three failures came from Crucible enforcement landing without the self-test being updated: the dry-run check runs the bundled `Phase-DOTNET-EXAMPLE.md`, which carries no `crucibleId`, so `runPlan` correctly refused it with `CRUCIBLE_ID_REQUIRED` and the assertions then dereferenced a missing `result.plan`. The naive fix ΓÇö passing `manualImport: true` ΓÇö was measured appending to `.forge/crucible/manual-imports.jsonl` on every run, which would fill a governance audit trail with self-test noise; and pointing `cwd` at a scratch directory is refused by the path-traversal guard requiring the plan to live inside the project. The check now copies the example into a temp project and runs there, so the bypass record lands in scratch and is discarded. The fourth failure was a stale assertion expecting `claude-opus-4.6` as the no-config routing default where the code returns `claude-opus-4.8`. Now **109 passed, 0 failed**, exit 0, repo audit trail untouched.
- **Watcher default model contradicted itself across the ACI surface.** `forge_watch`'s description said `claude-opus-4.8` while its own `model` parameter said `claude-opus-4.7`, and `tool-metadata.mjs` said 4.7 again ΓÇö agents read these to reason about cost and behaviour. The cause was four independent copies of one value: `DEFAULT_WATCHER_MODEL` declared separately in `hooks.mjs` and `review-watcher.mjs`, then restated twice in prose. Single-sourced to `orchestrator/constants.mjs` (a leaf module, no circular-import risk); both descriptions corrected. `server.mjs --check` caught the resulting artifact drift, which is exactly what it was added for.
- **Model pricing was wrong for six entries, two of them inverted flagship rates.** Every `MODEL_PRICING` entry is now taken from a vendor pricing page; previously three carried `estimated:` placeholders holding real dollar figures. **`gpt-5.6-sol` and `gpt-5.6-terra` had their rates swapped** ΓÇö sol is the flagship at $5/$30 and terra the mid tier at $2/$12, but the placeholders guessed the reverse, so sol under-billed 2├ù and terra over-billed 2.5├ù; `gpt-5.6-luna` over-billed 3.75├ù ($0.75/$4.50 guessed against $0.20/$1.20 actual). `gemini-3.5-flash` carried the Flash-**Lite** rate ($0.30/$2.50 against $1.50/$9.00 actual ΓÇö a 5├ù input understatement) and `gemini-3.1-pro-preview` carried Gemini 2.5 Pro's ($1.25/$5 against $2.00/$12). `o4-mini`'s `cache_read_multiplier` was `0.275`, which is the *cached price per MTok* rather than the ratio (0.25); `o3-mini`'s was 0.25 where the vendor ratio is 0.50. Unknown models fall back to a Sonnet-class $3/$15, so each of these was a silent distortion in the one tool whose purpose is preventing surprise spend.
- **Kimi models priced but not vendor-mapped.** `deriveVendorFromModel()` recognises `claude-` / `gpt-` / `o[1-9]` / `grok-` / `gemini-` prefixes and returns `null` otherwise, so the newly added `kimi-*` entries reported vendor `null` through telemetry and cost attribution ΓÇö a half-migration created by touching the pricing table without touching the vendor map. Added `kimi-* ΓåÆ moonshot`, plus a guard asserting **every** key in `MODEL_PRICING` resolves to a non-null vendor, so the next model added cannot repeat it. Worker routing needed no change: `probeQuorumModelAvailability` is prefix- and worker-availability-based with no model allowlist, and `claude-opus-5`, `claude-fable-5`, `kimi-k3`, and `gemini-3.6-flash` were each measured resolving identically to `claude-opus-4.8` (`via=cli`, `worker=gh-copilot`).
- **`modelRouting.default` advertised a stale closed enum.** The `.forge.json` schema published through `forge_capabilities` listed eleven models, none of them current flagships, so an agent reading the capability surface would conclude `claude-opus-5` was unsupported. `loadModelRouting()` never validated against it ΓÇö tests pass arbitrary values like `gpt-4o` ΓÇö making the list purely advisory and purely misleading. Refreshed, and now labelled as advisory with the runtime source of truth named.
- **OpenTelemetry tracing was silently inert whenever it was switched on, and is now unbundled.** `initOtel()` swallowed the dynamic-import error and returned `null`, so `OTEL_ENABLED=true` produced no traces and no diagnostic. That masked a packaging bug: `sdk-node` requires `@opentelemetry/api` as a **peer** (`>=1.3.0 <1.10.0`), Plan Forge declared it only in `optionalDependencies`, and npm silently drops it ΓÇö no lockfile entry, no error. Fixed both halves ([#238](https://github.com/srnichols/plan-forge/issues/238)): the catch now warns with the underlying error and the exact install command, and the three OTel packages are **removed from the shipped manifest** rather than promoted to real dependencies ΓÇö they are 77 packages / 89.2 MB for a feature gated off by default (Principle 7). `docs/observability/otel-schema.md` documents the opt-in install, including the non-obvious requirement to name `@opentelemetry/api` explicitly. Install tree drops **194 packages**. Verified end-to-end: with the packages installed, `initOtel()` returns a live `NodeSDK` with working tracer and meter ΓÇö which had never happened before.
- **MCP server boot silently stripped tool metadata from `tools.json`.** `buildCapabilityTools()` enriches via `TOOL_METADATA[name] || {}` and `writeToolsJson()` runs on every boot, so six registered tools with no metadata entry (`forge_classifier_issue`, `forge_export_plan`, `forge_github_metrics`, `forge_github_status`, `forge_patterns_list`, `forge_team_dashboard`) lost `intent`, `aliases`, `cost`, `risk`, `errors`, and `example` on every start. This fired on `node pforge-mcp/server.mjs --validate` ΓÇö the first line of the **Preferred gate pattern** in `step2-harden-plan.prompt.md` ΓÇö so the standard slice gate deleted 226 lines of tracked metadata and the orchestrator auto-commit swept it into unrelated slices ([#240](https://github.com/srnichols/plan-forge/issues/240)). Entries back-filled verbatim; `tools.json` now regenerates byte-identical. Added a bidirectional `TOOL_METADATA` Γåö `TOOL_NAMES` guard (the existing test only checked a hardcoded list of ten) plus a CI step asserting `tools.json` is reproducible from source.
- **GitHub issues filed by Plan Forge rendered a literal `\n` instead of line breaks.** `forge_meta_bug_file` and `forge_classifier_issue` built `gh issue create` as a **shell command string**, so newlines had to be escaped to survive it ΓÇö and `gh`, which does no escape processing, wrote the two-character sequence straight into the issue body. Every meta-bug and classifier-noise issue filed through the CLI path was affected (observed on [#236](https://github.com/srnichols/plan-forge/issues/236) and [#237](https://github.com/srnichols/plan-forge/issues/237)); the REST fallback was always correct, but `gh` is tried first. `gh` is now invoked with an **argv array** via `execFileSync`, so bodies survive verbatim.

### Changed

- **`forge_doctor_quorum` can now assess the models you actually configured** ([#243](https://github.com/srnichols/plan-forge/issues/243)). The tool is described as a preflight viability check, but its input was a closed enum of the two built-in presets and `assessQuorumViability()` resolved `QUORUM_PRESETS[name]`, so a user with custom `quorum.models` got a report about models they were not using. A typo like `gtp-5.6-sol` would pass the probe (`gh-copilot` is a catch-all), draw silent Sonnet-class fallback pricing, appear in no diagnostic, and fail at runtime after spend ΓÇö the exact hazard the existing preset guard warns about ("would fall back to default rates, silently distorting quorum estimates"), which covered only the built-ins. `preset: "config"` now assesses the effective `.forge.json` configuration, each model carries `priced`, and `warnings` names anything absent from the pricing registry. **The probe was deliberately left permissive**: restricting it to registry members would block every newly released model until the registry caught up, and that same permissiveness is why `claude-opus-5` worked on day one with no code change. An unpriced model is therefore reported and still `available`. The pricing predicate is injected rather than imported so `worker-spawn.mjs` stays free of `cost-service`.
- **Documented how to get Plan Forge past an enterprise MCP allowlist** ([#242](https://github.com/srnichols/plan-forge/issues/242)). Enterprise owners can now restrict which MCP servers Copilot runs via `allowedMcpServers` / `deniedMcpServers` in `copilot/managed-settings.json`. Two properties make the resulting failure nearly undiagnosable: the policy **fails closed**, and the settings live in the organisation's `.github-private` repository rather than on the developer's machine ΓÇö so setup completes, `.vscode/mcp.json` is correct, and no `forge_*` tools appear, with nothing local to inspect. A detector was deliberately **not** built: there is no local artifact to read, and a check living inside the MCP server cannot report the server being blocked. Local stdio servers are matched by `serverCommand` on *exact command and arguments*, so `COPILOT-VSCODE-GUIDE.md` now ships the literal allowlist entries for both `pforge-mcp/server.mjs` and `pforge-master/server.mjs`, which is more actionable than a detector would have been. Verified `setup.ps1` and `setup.sh` emit byte-identical `command`/`args`, since exact matching makes that parity load-bearing. Also restored a missing `###` heading that had left the instruction-file troubleshooting steps orphaned mid-page.
- **`test-smells` now catches duration-budget assertions, and three latent flakes were fixed.** The scanner's `TIME-FLAKE` rule matched only *sources* of time (`Date.now`, `setTimeout`, `performance.now`), so `expect(result.durationMs).toBeLessThan(1000)` ΓÇö the same flake class wearing the clothes of an ordinary numeric assertion ΓÇö scored **zero findings** while `search-core` was failing full runs and passing 41/41 solo. The new pattern is deliberately narrow, because a scanner that cries wolf gets muted: `toBeLessThan` only (the direction that fails under load ΓÇö `toBeGreaterThanOrEqual(0)` on a duration cannot fail at all), numeric literals only (so the correct relative form `toBeLessThanOrEqual(r1.durationMs + 50)` stays clean), and `runtime` excluded from the name list because it collides with `runtimes.length` and `runtimeBudgetMs`. Two guard-mechanism defects surfaced while tuning it: the context window was 250 chars, too small to see a multi-line rationale sitting six lines above the assertion (now 500), and `budget` had to be rejected as a guard word because it appears inside identifiers. Findings went 8 ΓåÆ 2 as precision improved, and both survivors were genuine: `timeline-core` (10k-record O(n┬▓) guard) and `brain-remember` (proving a call does not block) now carry stated tolerances. Zero unguarded budgets remain. The scanner also gained an entry guard ΓÇö importing it previously ran a full scan and could set a non-zero exit code.
- **Model registry refreshed to current vendor line-ups** ΓÇö 56 ΓåÆ 62 entries, zero estimates remaining. Added `claude-opus-5` ($5/$25) and `claude-fable-5` ($10/$50, the first Anthropic tier above Opus), `gemini-3.6-flash` ($1.50/$7.50) and `gemini-3.5-flash-lite`, and the Moonshot Kimi family ΓÇö `kimi-k3` ($3/$15, 1M ctx), `kimi-k2.7-code`, and `kimi-k2.7-code-highspeed`, whose vendor publishes cache-hit and cache-miss input rates directly, giving the 0.10├ù and 0.20├ù read multipliers by division. Gemini 3.x entries also gained the 0.10├ù cache-read multiplier they had been missing, having defaulted to 1.0├ù ΓÇö no cache benefit assumed. Two vendor behaviours are recorded in `_source` but deliberately **not** modelled, because modelling them is a billing-math change rather than a data refresh: GPT-5.6 now bills cache **writes** at 1.25├ù input (`priceSlice()` computes cache-write cost only on the Anthropic branch, so adding the field to an OpenAI entry would be inert), and both GPT-5.6 and Gemini Pro bill a higher rate above a context threshold (272k and 200k respectively) where entries carry the lower-tier rate.
- **Quorum defaults left on `claude-opus-4.7` / `claude-opus-4.8`.** Anthropic now lists both as legacy and recommends Opus 5, and the price is identical, so the change is tempting ΓÇö but those identifiers appear across 32 files including preset tables, watcher defaults, REST handlers, and roughly twenty test files that assert the exact strings, and whether `claude-opus-5` resolves through the worker-CLI routing (`probeQuorumModelAvailability`, `assertWorkerBackendReady`) could not be verified from here. It is a migration with a runtime-availability risk, not a release-eve edit. Opus 5 is fully usable today by setting `quorum.models` in `.forge.json`, and now prices correctly when you do.
- **All 5 blocking clean-code `complexity-error` violations cleared** ΓÇö repo-wide count is now **0 errors across 651 files** (was flagged in `cleanup-findings/CATEGORIES-SUMMARY.md` as blocking before a release gate). `parseGrokStreamingJson` 35, `assertWorkerBackendReady` 29, `renderBody` 29, `loadQuorumConfig` 23, and the `forge_master_audit` handler 46 are all under the limit of 20. Behaviour-preserving throughout, verified by the existing suites including the byte-exact `bug-batch` golden fixture and the real grok streaming-json capture.
- **`engines.node` declared for the first time.** All three manifests had no `engines` field, so an unsupported Node produced cryptic syntax errors. The floors differ by audience: published packages declare `>=18.0.0` (matching the shipped API floor ΓÇö `globalThis.fetch`, `AbortSignal.timeout`, and no `import.meta.dirname` outside tests), the private root declares `>=20.11.0` (the test suite uses `import.meta.dirname` in 17 files). `AGENTS.md` said only "Node.js 18+", correct for consumers but wrong for contributors; it now states both.
- **New shared module `pforge-mcp/tempering/gh-cli.mjs`** ΓÇö `buildCreateIssueArgs`, `createIssueViaGhCli`, `findOpenIssueByHashViaGhCli`. The issue-create and dedupe-lookup logic was duplicated byte-for-byte across the meta-bug filer and the classifier filer, differing only by label; both now delegate. Callers inject `execFile`; `execSync` is retained for `gh auth token` and `git remote`, which carry no user input.
- **Plan hardener now requires premise verification** (`step2-harden-plan.prompt.md`). A plan's Non-Goals asserted that all 17 API endpoints already existed; the target route file had only `approve` and `deny`, so a slice was scoped against a surface that was never there ([#237](https://github.com/srnichols/plan-forge/issues/237)). Self-check item 10 plus a **Premise Verification** section now require every factual claim about existing code to be checked by reading the file ΓÇö false assertions get the slice re-scoped, unverifiable ones get demoted from Non-Goals to Required Decisions so they never stand as a scope boundary.
- **Dropped the unused `@opentelemetry/exporter-trace-otlp-http` declaration** ΓÇö never imported anywhere, and it pinned an old 0.57 copy that `sdk-node` already supplies at 0.221.0.
- **`smith-golden` spawn budget raised** 60s ΓåÆ 150s (vitest timeout 90s ΓåÆ 180s). It is the only test that shells out to a complete `pwsh` CLI run: ~3.9s solo, but the suite runs ~340s of test time in ~43s wall (Γëê8├ù parallelism), so the old budget was intermittently exceeded under saturation and produced a spurious single-file failure in roughly half of full-suite runs.
- **`autocannon` unbundled.** It sat in `dependencies` of the published `plan-forge-mcp` package, so every consumer installed it plus **31 transitive packages** to support one opt-in tempering scanner most projects never run. `load-stress.mjs` already lazy-imports it and already degraded gracefully, so the absence path was designed and tested long before this change ΓÇö it just returned a bare `autocannon-import-failed` code with no way to know an opt-in install was needed, so it now carries an actionable message. Same shape as the OTel unbundling. The technology choice is unchanged; `PROJECT-PRINCIPLES.md` records both as not bundled.
- **`OTEL_LOG_LEVEL` documented** (`docs/observability/otel-schema.md`). `OTEL_ENABLED=true` alone is a **valid** setup ΓÇö with no `OTEL_EXPORTER_OTLP_ENDPOINT` the SDK targets the standard OTLP default `localhost:4318`, which works when a collector is listening there. When one is not, spans are dropped and the SDK stays quiet, because export failures go to OpenTelemetry's `diag` channel, which is a no-op until a logger is registered. Measured over a 20 s window: the default configuration emits nothing, while `OTEL_LOG_LEVEL=error` surfaces the `ECONNREFUSED` and the endpoint it tried. No code change ΓÇö the SDK already honours the variable, and Plan Forge deliberately does not register a `diag` logger so the knob stays the operator's.
- **`server.mjs --check`** ΓÇö a read-only twin of `--validate`. `--validate` **regenerates** `tools.json` and `cli-schema.json`, and it was the first line of the "Preferred gate pattern" in `step2-harden-plan.prompt.md`, so the standard slice gate mutated tracked source and the orchestrator's auto-commit swept the change into unrelated slices ΓÇö the mechanism behind [#240](https://github.com/srnichols/plan-forge/issues/240). `--check` verifies the same artifacts, reports `missing` / `stale` per file, exits non-zero on drift, and never writes. The preferred gate and `capabilities-drift.yml` now use it; `--validate` keeps its documented regeneration role for when the tool surface actually changes.
...
- **`npm audit` now reports 0 vulnerabilities** (from 72 at the start of this cycle, 1 critical / 19 high). The last three moderates came from `autocannon` → `hyperid` → `uuid`, and are now gone because **`autocannon` is no longer bundled** — see Changed. Worth recording that those three were never reachable: the advisory ([GHSA-w5hq-g745-h8pq](https://github.com/advisories/GHSA-w5hq-g745-h8pq)) is a missing buffer bounds check in uuid **v3/v5/v6 when `buf` is provided**, while `hyperid` calls `crypto.randomUUID` on modern Node and falls back to `uuid.v4` otherwise — neither path reaches the vulnerable functions. `npm audit fix` proposed `autocannon@2.0.1`, a **six-major downgrade**, because 8.0.0 is already the latest and no forward fix exists.
- **`vitest` 2.1.9 → 4.1.10** ([#239](https://github.com/srnichols/plan-forge/issues/239)) clears the last **critical** advisory (`vite`, via vitest) plus the remaining high. The 36 measured failures resolved to three causes, not thirty-six: variable-specifier dynamic imports (vitest 4 resolves a non-literal specifier against the project root rather than the importing module — 26 tests, including a latent fragility in the production `loadAdapter`), a `testTimeout` default too small for a suite that shells out to real CLI probes, and two contention-only perf budgets. **No test assertion was weakened** — the `cost-service`/`estimate-slice` parity and `quorum-probe` availability tests flagged as masking risks all pass on their original assertions.
- **`sharp` and the OpenTelemetry SDK upgraded off high-severity advisories.** `sharp` 0.34.5 → 0.35.3 (native module — all five conversion formats `jpg`/`png`/`webp`/`avif`/`gif` were exercised directly against libvips 8.18.3, since a green unit suite does not prove a native rebuild works). `@opentelemetry/sdk-node` 0.57.2 → 0.221.0 and `auto-instrumentations-node` 0.56.1 → 0.79.0.
- **`ws` high-severity advisories patched.** `ws` was pinned at 8.20.0 — inside the vulnerable `8.0.0 - 8.20.1` range for [GHSA-58qx-3vcg-4xpx](https://github.com/advisories/GHSA-58qx-3vcg-4xpx) (uninitialized memory disclosure) and [GHSA-96hv-2xvq-fx4p](https://github.com/advisories/GHSA-96hv-2xvq-fx4p) (memory-exhaustion DoS from tiny fragments). It is a direct dependency of both `pforge-mcp` and `pforge-master` and backs the dashboard WebSocket, so it was reachable in a server users actually run. Now 8.21.3, along with non-breaking fixes for 13 transitive highs. Lockfile only — no `package.json` ranges changed.
...
- **Crucible finalize claimed `Phase-01` in a repository already at Phase-59** ([#244](https://github.com/srnichols/plan-forge/issues/244)). `PHASE_NAME_RE` matches only a bare `Phase-NN`, and `collectExistingPhaseNames()` filtered plan basenames through it — so every plan using this project's own `Phase-NN-NAME-PLAN.md` convention was discarded, the list came back empty, and `nextPhaseNumber([])` returned `Phase-01`. Not a one-off: each subsequent smelt would keep claiming low numbers while real history sat in the fifties. Fixed at the collection layer with `phaseNameFromPlanFile()`, which recovers the canonical `Phase-NN` from a suffixed filename; `isValidPhaseName`, `nextPhaseNumber` and `claimPhaseNumber` keep their existing semantics, so the test asserting `Phase-CRUCIBLE-01` is *not* a numbered phase stays correct and untouched. The repo now reports 14 numbered phases where it previously reported none. Found by using the tool: the smelt for Phase-60 was written as `Phase-01.md`, and the hand-picked replacement number was itself wrong because it was eyeballed from a truncated listing — precisely the error the automated claim exists to prevent.
- **Crucible tool surface: unbounded payload, unconfirmed lane, malformed frontmatter** ([#245](https://github.com/srnichols/plan-forge/issues/245)). Three defects found while running a 13-question interview. `forge_crucible_ask` echoed the entire draft on every call, and the draft grows with each answer — measured at 9 KB rising to 24 KB across one interview, against an ACI budget of roughly 10 KB, and redundant because `forge_crucible_preview` already exists for on-demand review. The draft is now opt-in via `includeDraft`, with a `draftHint` naming the preview tool; the dashboard's REST route opts in so its live-preview pane is unaffected. A default response is now ~357 bytes regardless of interview length. `forge_crucible_submit` echoed only `recommendedLane`, so submitting with `mode:"full"` still reported `"tweak"` and an agent could not tell whether its override applied — it now returns `lane` (applied) alongside `recommendedLane` (heuristic). And the `linked-bugs` answer was split on commas and interpolated into a YAML flow sequence, so a prose reply emitted `linkedBugs: [free text with colons and #241 …]`; inside a flow sequence an unquoted ` #` opens a comment. Tokens are now kept only when they are tracker-shaped (`RMG-0035`) — the first attempt at this filter was too permissive and matched ordinary words like "research", which the tests caught.
- **`pforge` self-test reported 4 false failures and exited 1.** `node pforge-mcp/orchestrator.mjs --test` finished **105 passed, 4 failed** — a shipped diagnostic telling every user the tool is broken. Two independent causes. Three failures came from Crucible enforcement landing without the self-test being updated: the dry-run check runs the bundled `Phase-DOTNET-EXAMPLE.md`, which carries no `crucibleId`, so `runPlan` correctly refused it with `CRUCIBLE_ID_REQUIRED` and the assertions then dereferenced a missing `result.plan`. The naive fix — passing `manualImport: true` — was measured appending to `.forge/crucible/manual-imports.jsonl` on every run, which would fill a governance audit trail with self-test noise; and pointing `cwd` at a scratch directory is refused by the path-traversal guard requiring the plan to live inside the project. The check now copies the example into a temp project and runs there, so the bypass record lands in scratch and is discarded. The fourth failure was a stale assertion expecting `claude-opus-4.6` as the no-config routing default where the code returns `claude-opus-4.8`. Now **109 passed, 0 failed**, exit 0, repo audit trail untouched.
- **Watcher default model contradicted itself across the ACI surface.** `forge_watch`'s description said `claude-opus-4.8` while its own `model` parameter said `claude-opus-4.7`, and `tool-metadata.mjs` said 4.7 again — agents read these to reason about cost and behaviour. The cause was four independent copies of one value: `DEFAULT_WATCHER_MODEL` declared separately in `hooks.mjs` and `review-watcher.mjs`, then restated twice in prose. Single-sourced to `orchestrator/constants.mjs` (a leaf module, no circular-import risk); both descriptions corrected. `server.mjs --check` caught the resulting artifact drift, which is exactly what it was added for.
- **Model pricing was wrong for six entries, two of them inverted flagship rates.** Every `MODEL_PRICING` entry is now taken from a vendor pricing page; previously three carried `estimated:` placeholders holding real dollar figures. **`gpt-5.6-sol` and `gpt-5.6-terra` had their rates swapped** — sol is the flagship at $5/$30 and terra the mid tier at $2/$12, but the placeholders guessed the reverse, so sol under-billed 2× and terra over-billed 2.5×; `gpt-5.6-luna` over-billed 3.75× ($0.75/$4.50 guessed against $0.20/$1.20 actual). `gemini-3.5-flash` carried the Flash-**Lite** rate ($0.30/$2.50 against $1.50/$9.00 actual — a 5× input understatement) and `gemini-3.1-pro-preview` carried Gemini 2.5 Pro's ($1.25/$5 against $2.00/$12). `o4-mini`'s `cache_read_multiplier` was `0.275`, which is the *cached price per MTok* rather than the ratio (0.25); `o3-mini`'s was 0.25 where the vendor ratio is 0.50. Unknown models fall back to a Sonnet-class $3/$15, so each of these was a silent distortion in the one tool whose purpose is preventing surprise spend.
- **Kimi models priced but not vendor-mapped.** `deriveVendorFromModel()` recognises `claude-` / `gpt-` / `o[1-9]` / `grok-` / `gemini-` prefixes and returns `null` otherwise, so the newly added `kimi-*` entries reported vendor `null` through telemetry and cost attribution — a half-migration created by touching the pricing table without touching the vendor map. Added `kimi-* → moonshot`, plus a guard asserting **every** key in `MODEL_PRICING` resolves to a non-null vendor, so the next model added cannot repeat it. Worker routing needed no change: `probeQuorumModelAvailability` is prefix- and worker-availability-based with no model allowlist, and `claude-opus-5`, `claude-fable-5`, `kimi-k3`, and `gemini-3.6-flash` were each measured resolving identically to `claude-opus-4.8` (`via=cli`, `worker=gh-copilot`).
- **`modelRouting.default` advertised a stale closed enum.** The `.forge.json` schema published through `forge_capabilities` listed eleven models, none of them current flagships, so an agent reading the capability surface would conclude `claude-opus-5` was unsupported. `loadModelRouting()` never validated against it — tests pass arbitrary values like `gpt-4o` — making the list purely advisory and purely misleading. Refreshed, and now labelled as advisory with the runtime source of truth named.
- **OpenTelemetry tracing was silently inert whenever it was switched on, and is now unbundled.** `initOtel()` swallowed the dynamic-import error and returned `null`, so `OTEL_ENABLED=true` produced no traces and no diagnostic. That masked a packaging bug: `sdk-node` requires `@opentelemetry/api` as a **peer** (`>=1.3.0 <1.10.0`), Plan Forge declared it only in `optionalDependencies`, and npm silently drops it — no lockfile entry, no error. Fixed both halves ([#238](https://github.com/srnichols/plan-forge/issues/238)): the catch now warns with the underlying error and the exact install command, and the three OTel packages are **removed from the shipped manifest** rather than promoted to real dependencies — they are 77 packages / 89.2 MB for a feature gated off by default (Principle 7). `docs/observability/otel-schema.md` documents the opt-in install, including the non-obvious requirement to name `@opentelemetry/api` explicitly. Install tree drops **194 packages**. Verified end-to-end: with the packages installed, `initOtel()` returns a live `NodeSDK` with working tracer and meter — which had never happened before.
- **MCP server boot silently stripped tool metadata from `tools.json`.** `buildCapabilityTools()` enriches via `TOOL_METADATA[name] || {}` and `writeToolsJson()` runs on every boot, so six registered tools with no metadata entry (`forge_classifier_issue`, `forge_export_plan`, `forge_github_metrics`, `forge_github_status`, `forge_patterns_list`, `forge_team_dashboard`) lost `intent`, `aliases`, `cost`, `risk`, `errors`, and `example` on every start. This fired on `node pforge-mcp/server.mjs --validate` — the first line of the **Preferred gate pattern** in `step2-harden-plan.prompt.md` — so the standard slice gate deleted 226 lines of tracked metadata and the orchestrator auto-commit swept it into unrelated slices ([#240](https://github.com/srnichols/plan-forge/issues/240)). Entries back-filled verbatim; `tools.json` now regenerates byte-identical. Added a bidirectional `TOOL_METADATA` ↔ `TOOL_NAMES` guard (the existing test only checked a hardcoded list of ten) plus a CI step asserting `tools.json` is reproducible from source.
...
- **`forge_doctor_quorum` can now assess the models you actually configured** ([#243](https://github.com/srnichols/plan-forge/issues/243)). The tool is described as a preflight viability check, but its input was a closed enum of the two built-in presets and `assessQuorumViability()` resolved `QUORUM_PRESETS[name]`, so a user with custom `quorum.models` got a report about models they were not using. A typo like `gtp-5.6-sol` would pass the probe (`gh-copilot` is a catch-all), draw silent Sonnet-class fallback pricing, appear in no diagnostic, and fail at runtime after spend — the exact hazard the existing preset guard warns about ("would fall back to default rates, silently distorting quorum estimates"), which covered only the built-ins. `preset: "config"` now assesses the effective `.forge.json` configuration, each model carries `priced`, and `warnings` names anything absent from the pricing registry. **The probe was deliberately left permissive**: restricting it to registry members would block every newly released model until the registry caught up, and that same permissiveness is why `claude-opus-5` worked on day one with no code change. An unpriced model is therefore reported and still `available`. The pricing predicate is injected rather than imported so `worker-spawn.mjs` stays free of `cost-service`.
- **Documented how to get Plan Forge past an enterprise MCP allowlist** ([#242](https://github.com/srnichols/plan-forge/issues/242)). Enterprise owners can now restrict which MCP servers Copilot runs via `allowedMcpServers` / `deniedMcpServers` in `copilot/managed-settings.json`. Two properties make the resulting failure nearly undiagnosable: the policy **fails closed**, and the settings live in the organisation's `.github-private` repository rather than on the developer's machine — so setup completes, `.vscode/mcp.json` is correct, and no `forge_*` tools appear, with nothing local to inspect. A detector was deliberately **not** built: there is no local artifact to read, and a check living inside the MCP server cannot report the server being blocked. Local stdio servers are matched by `serverCommand` on *exact command and arguments*, so `COPILOT-VSCODE-GUIDE.md` now ships the literal allowlist entries for both `pforge-mcp/server.mjs` and `pforge-master/server.mjs`, which is more actionable than a detector would have been. Verified `setup.ps1` and `setup.sh` emit byte-identical `command`/`args`, since exact matching makes that parity load-bearing. Also restored a missing `###` heading that had left the instruction-file troubleshooting steps orphaned mid-page.
- **`test-smells` now catches duration-budget assertions, and three latent flakes were fixed.** The scanner's `TIME-FLAKE` rule matched only *sources* of time (`Date.now`, `setTimeout`, `performance.now`), so `expect(result.durationMs).toBeLessThan(1000)` — the same flake class wearing the clothes of an ordinary numeric assertion — scored **zero findings** while `search-core` was failing full runs and passing 41/41 solo. The new pattern is deliberately narrow, because a scanner that cries wolf gets muted: `toBeLessThan` only (the direction that fails under load — `toBeGreaterThanOrEqual(0)` on a duration cannot fail at all), numeric literals only (so the correct relative form `toBeLessThanOrEqual(r1.durationMs + 50)` stays clean), and `runtime` excluded from the name list because it collides with `runtimes.length` and `runtimeBudgetMs`. Two guard-mechanism defects surfaced while tuning it: the context window was 250 chars, too small to see a multi-line rationale sitting six lines above the assertion (now 500), and `budget` had to be rejected as a guard word because it appears inside identifiers. Findings went 8 → 2 as precision improved, and both survivors were genuine: `timeline-core` (10k-record O(n²) guard) and `brain-remember` (proving a call does not block) now carry stated tolerances. Zero unguarded budgets remain. The scanner also gained an entry guard — importing it previously ran a full scan and could set a non-zero exit code.
- **Model registry refreshed to current vendor line-ups** — 56 → 62 entries, zero estimates remaining. Added `claude-opus-5` ($5/$25) and `claude-fable-5` ($10/$50, the first Anthropic tier above Opus), `gemini-3.6-flash` ($1.50/$7.50) and `gemini-3.5-flash-lite`, and the Moonshot Kimi family — `kimi-k3` ($3/$15, 1M ctx), `kimi-k2.7-code`, and `kimi-k2.7-code-highspeed`, whose vendor publishes cache-hit and cache-miss input rates directly, giving the 0.10× and 0.20× read multipliers by division. Gemini 3.x entries also gained the 0.10× cache-read multiplier they had been missing, having defaulted to 1.0× — no cache benefit assumed. Two vendor behaviours are recorded in `_source` but deliberately **not** modelled, because modelling them is a billing-math change rather than a data refresh: GPT-5.6 now bills cache **writes** at 1.25× input (`priceSlice()` computes cache-write cost only on the Anthropic branch, so adding the field to an OpenAI entry would be inert), and both GPT-5.6 and Gemini Pro bill a higher rate above a context threshold (272k and 200k respectively) where entries carry the lower-tier rate.
- **Quorum defaults left on `claude-opus-4.7` / `claude-opus-4.8`.** Anthropic now lists both as legacy and recommends Opus 5, and the price is identical, so the change is tempting — but those identifiers appear across 32 files including preset tables, watcher defaults, REST handlers, and roughly twenty test files that assert the exact strings, and whether `claude-opus-5` resolves through the worker-CLI routing (`probeQuorumModelAvailability`, `assertWorkerBackendReady`) could not be verified from here. It is a migration with a runtime-availability risk, not a release-eve edit. Opus 5 is fully usable today by setting `quorum.models` in `.forge.json`, and now prices correctly when you do.
- **All 5 blocking clean-code `complexity-error` violations cleared** — repo-wide count is now **0 errors across 651 files** (was flagged in `cleanup-findings/CATEGORIES-SUMMARY.md` as blocking before a release gate). `parseGrokStreamingJson` 35, `assertWorkerBackendReady` 29, `renderBody` 29, `loadQuorumConfig` 23, and the `forge_master_audit` handler 46 are all under the limit of 20. Behaviour-preserving throughout, verified by the existing suites including the byte-exact `bug-batch` golden fixture and the real grok streaming-json capture.
- **`engines.node` declared for the first time.** All three manifests had no `engines` field, so an unsupported Node produced cryptic syntax errors. The floors differ by audience: published packages declare `>=18.0.0` (matching the shipped API floor — `globalThis.fetch`, `AbortSignal.timeout`, and no `import.meta.dirname` outside tests), the private root declares `>=20.11.0` (the test suite uses `import.meta.dirname` in 17 files). `AGENTS.md` said only "Node.js 18+", correct for consumers but wrong for contributors; it now states both.
...
- **Dropped the unused `@opentelemetry/exporter-trace-otlp-http` declaration** — never imported anywhere, and it pinned an old 0.57 copy that `sdk-node` already supplies at 0.221.0.
- **`smith-golden` spawn budget raised** 60s → 150s (vitest timeout 90s → 180s). It is the only test that shells out to a complete `pwsh` CLI run: ~3.9s solo, but the suite runs ~340s of test time in ~43s wall (≈8× parallelism), so the old budget was intermittently exceeded under saturation and produced a spurious single-file failure in roughly half of full-suite runs.
- **`autocannon` unbundled.** It sat in `dependencies` of the published `plan-forge-mcp` package, so every consumer installed it plus **31 transitive packages** to support one opt-in tempering scanner most projects never run. `load-stress.mjs` already lazy-imports it and already degraded gracefully, so the absence path was designed and tested long before this change — it just returned a bare `autocannon-import-failed` code with no way to know an opt-in install was needed, so it now carries an actionable message. Same shape as the OTel unbundling. The technology choice is unchanged; `PROJECT-PRINCIPLES.md` records both as not bundled.
- **`OTEL_LOG_LEVEL` documented** (`docs/observability/otel-schema.md`). `OTEL_ENABLED=true` alone is a **valid** setup — with no `OTEL_EXPORTER_OTLP_ENDPOINT` the SDK targets the standard OTLP default `localhost:4318`, which works when a collector is listening there. When one is not, spans are dropped and the SDK stays quiet, because export failures go to OpenTelemetry's `diag` channel, which is a no-op until a logger is registered. Measured over a 20 s window: the default configuration emits nothing, while `OTEL_LOG_LEVEL=error` surfaces the `ECONNREFUSED` and the endpoint it tried. No code change — the SDK already honours the variable, and Plan Forge deliberately does not register a `diag` logger so the knob stays the operator's.
- **`server.mjs --check`** — a read-only twin of `--validate`. `--validate` **regenerates** `tools.json` and `cli-schema.json`, and it was the first line of the "Preferred gate pattern" in `step2-harden-plan.prompt.md`, so the standard slice gate mutated tracked source and the orchestrator's auto-commit swept the change into unrelated slices — the mechanism behind [#240](https://github.com/srnichols/plan-forge/issues/240). `--check` verifies the same artifacts, reports `missing` / `stale` per file, exits non-zero on drift, and never writes. The preferred gate and `capabilities-drift.yml` now use it; `--validate` keeps its documented regeneration role for when the tool surface actually changes.
...
Offline access to the Plan Forge trajectory notes (`.forge/trajectories/`) without requiring a running MCP server.

#### Added
- `pforge-sdk/src/trajectory-reader.mjs` — zero-dependency reader module:
  - `trajectoriesDir({ cwd? })` — resolve `.forge/trajectories/` path.
  - `planTrajectoriesDir({ planBasename, cwd? })` — resolve per-plan directory path. Guards against path traversal.
  - `trajectoryFilePath({ planBasename, sliceId, cwd? })` — resolve path to a specific `slice-N.md` file.
  - `parseSliceId(filename)` — extract slice ID from `slice-N.md`; returns `null` for non-matching input.
  - `listPlans({ cwd? })` — list plan basenames that have trajectory directories, sorted alphabetically.
  - `listTrajectories({ planBasename, cwd? })` — list `{ sliceId, path, wordCount }` entries sorted numerically.
  - `readTrajectory({ planBasename, sliceId, cwd? })` — read `{ sliceId, content, wordCount }` for a single slice; returns `null` for missing/unreadable files.
  - `readAllTrajectories({ planBasename, cwd? })` — read all slices in ascending slice-ID order.
  - `countWords(text)` — whitespace-delimited word count; zero for non-string input.
  - `summarizeTrajectories(entries)` — pure `{ total, sliceIds, totalWords, avgWords }` from an in-memory array.
  - Constants: `TRAJECTORIES_DIR_RELATIVE`.
...
- **[A1]** Forbidden Actions matcher hardening: expanded to support glob patterns (`docs/**/*.html`), directory patterns (`pforge-mcp/`), and intent-style entries (`**Do NOT**` lines).
- **[A2]** `forge_diff_classify` tool: scans staged git diffs for 6 categories (`leaked-secret`, `prompt-injection-echo`, `eval-exec-introduced`, `license-incompatible-paste`, `unexpected-network-call`, `large-binary-dump`); blocks on severity ≥ high; wired into PreCommit chain.
- **[A3]** PreCommit chain framework: `PreCommit.mjs` now reads `hooks.preCommit.chain[]` from `.forge.json`; master-branch-reject is the first chain entry; first non-zero exit aborts the chain.
- **[A4]** `plan-health-auditor.agent.md`: new read-only agent reads run history + memories + bugs and emits markdown report to `.forge/health/latest.md`; invoke via `forge_master_ask({ message: "@plan-health-auditor weekly report" })`.
- **[A5]** `network.allowed` frontmatter (log-only): orchestrator spawns in-process HTTPS proxy when `network.allowed: [...]` is set; hosts logged to `.forge/runs/<run>/slices/<n>/network.log`; env `PFORGE_NETWORK_LOG_ONLY=1`.
- **[A6]** `lockHash` frontmatter: SHA-256 hash over plan body; orchestrator refuses to run if plan has drifted; Step-2 hardener now emits `lockHash:` as its final step.
- **[A7]** `--objective` mode on `forge_tempering_run`: accepts `objective.command` + `objective.acceptIf`; captures baseline metric before worker runs; rejects candidate if metric doesn't improve.
- **[A8]** `tools.deny` frontmatter: bridge strips listed MCP tools from worker session at init; worker never sees denied tools exist; defaults to empty (no-op).
