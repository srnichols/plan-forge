...
- **`vitest` 2.1.9 → 4.1.10 deferred** ([#239](https://github.com/srnichols/plan-forge/issues/239)). Fixes the last critical advisory, but it is dev-only (never shipped). Measured: **36 tests across 11 files fail** on v4 — 20 in `notifications-stubs.test.mjs` alone (variable-specifier dynamic imports vs. Vite 7's module runner), plus scattered failures in `tempering-integration`, `quorum-probe`, `cost-service` parity, and routing tests. Likely 3–5 root causes rather than 36, but the parity and probe failures could reflect genuine mock-semantics changes, so this needs a focused migration rather than batch test edits. Note `--reporter=basic` was removed in v3+ and will need replacing in any gates that use it.
- **`autocannon` moderate advisories: no action.** npm proposes `autocannon@2.0.1` as the "fix", which is a **downgrade** from the current 8.0.0. The vulnerability is a transitive `hyperid` → `uuid` moderate in a load-testing tool that is lazily imported only when the load-stress scanner runs. Downgrading six majors of a runtime dependency is a worse trade than the moderate.
