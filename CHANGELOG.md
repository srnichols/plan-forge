...
- **`server.mjs --validate` writes rather than checks.** It regenerates `tools.json` as a side effect, which is surprising for a validation command and is why [#240](https://github.com/srnichols/plan-forge/issues/240) went unnoticed for so long. The new CI check detects the damage but does not prevent it; a non-writing `--check` mode (mirroring `scripts/generate-capabilities-doc.mjs --check`) would remove the failure class.
- **`OTEL_ENABLED=true` without `OTEL_EXPORTER_OTLP_ENDPOINT` crashes on flush.** With the OTel packages installed and no collector listening, the exporter defaults to `localhost:4318` and the process dies with an uncaught `AggregateError [ECONNREFUSED]`. This is OTel SDK behaviour rather than Plan Forge logic, but `docs/observability/otel-schema.md` presents `OTEL_ENABLED` as a standalone activation gate, so following it literally yields a crash instead of a no-op.
