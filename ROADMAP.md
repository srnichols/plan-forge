...
**v2.70.0** (2026-04-23) — Forge-Master Runtime Observability (Phase-36). Classification events now forwarded via SSE (`event: classification` with `{ lane, confidence }`). Probe harness (`scripts/probe-forge-master.mjs`) captures classification per-probe and renders a "Classification match" accuracy table in the Markdown report. Console output shows `lane=<lane> conf=<conf>` inline. Validated: 14/21 lane-match, 70 lines ✅/OK in report. Results committed to `.forge/validation/`. Phase-36 Slice 4 shipped.
