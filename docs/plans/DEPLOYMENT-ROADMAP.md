...
| AUTH-RBAC-SCAFFOLD | Pluggable auth model + config-driven RBAC scaffold: `pforge-mcp/auth/` (bearer provider, SSO stub, RBAC resolver, middleware), `withAuth` wired into MCP tool dispatch + bridge edits, open-by-default when `.forge/rbac.json` absent. Three security docs + `.forge/rbac.example.json`. | 2026-05-20 | [Phase-AUTH-RBAC-SCAFFOLD-PLAN.md](./archive/Phase-AUTH-RBAC-SCAFFOLD-PLAN.md) |
