...
4. Create `pforge-master/src/principles.mjs`:
   ...
   - If all three sources are absent/empty, emit the universal baseline — a hardcoded const `UNIVERSAL_BASELINE` containing the nine-principle block verbatim from the Phase-32 OUTLINE's "The Philosophy Block" section (including the Work Triage Order triage with hotfix > operational > strategic ranking and the inversion-trap paragraph).
