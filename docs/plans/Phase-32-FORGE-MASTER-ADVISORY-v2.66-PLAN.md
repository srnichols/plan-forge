...
| 5 | Fallback principles when no source found | Resolved | **Universal architecture-first baseline** — the 5-question framework text pulled verbatim from [architecture-principles.instructions.md](.github/instructions/architecture-principles.instructions.md) (the "5 Questions Before Writing ANY Code" section). Ensures Forge-Master always has *some* principle to anchor its advisory response on. |
...
4. Create `pforge-master/src/principles.mjs`:
   ...
   - If all three sources are absent/empty, emit the universal baseline — a hardcoded const `UNIVERSAL_BASELINE` containing the "5 Questions Before Writing ANY Code" section distilled from `architecture-principles.instructions.md`.
