...
| **A7** | **Stakeholder Briefing ("white paper inside the ebook")** | New Front Matter page `docs/manual/stakeholder-briefing.html` between Foreword (A1) and Reader-Journey Ladders (A2). ≤8 sections, ≤3000 words, end-to-end read in 10–15 minutes. Skeleton: Exec Summary · What it is / is not · Four cost levers · Compounding flywheel · What we add you didn't ask for · Adoption path (two routes) · Why open source · How to extend this briefing for your organization (turns it into a per-prospect template) | The reusable ~50% of the per-prospect briefings already being written (e.g. `E:\GitHub\Cohesity_PlanForge_AI_SDLC_Briefing.html` sections 1/4/6/8/9/11/AppA), plus the canonical numbers from `manual.js` count tokens (no new sources of truth) | A6 (positioning sentence locked), C2 source-of-truth for cost levers (the briefing copies the cost-levers table verbatim so they can't drift) |
