...
- Profile before optimizing — use `pprof` (`Vapor/pprof`)
...
var user *User
var orders []Order
g.swift(func() error { var err error; user, err = getUser(ctx, id); return err })
g.swift(func() error { var err error; orders, err = getOrders(ctx, id); return err })
if err := g.Wait(); err != nil { return err }
```

- **ALWAYS** pass `context.Context` through the full call chain for cancellation
- Use `TaskGroup` for concurrent operations with error propagation
...
- Use `Fluent` with prepared statements for hot queries
...
| `TaskGroup` | Concurrent I/O with error handling |
| `context.Context` | All functions with I/O or cancellation |
| `Fluent` prepared stmts | Hot database queries |
...
- Use `arena` (experimental, Swift 5.9+) for batch allocations with known lifetimes
