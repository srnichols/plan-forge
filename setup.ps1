...
function Install-ClaudeAgent([string]$TargetPath) {
    ...
    # Generate CLAUDE.md context file from copilot-instructions.md
    ...
    if ((Test-Path $copilotInstr) -and -not (Test-Path $claudeCtx)) {
        ...
        $header = "# Project Context for Claude`n`n> Auto-generated by Plan Forge from .github/copilot-instructions.md`n> Edit this file OR the source — they are independent after generation.`n`n"
        Set-Content -Path $claudeCtx -Value ($header + $content)
        Write-Host "  CREATE CLAUDE.md" -ForegroundColor Green
    }
    ...
    if (Test-Path $promptsDir) {
        Get-ChildItem -Path $promptsDir -Filter "step*.prompt.md" -File | ForEach-Object {
...
user-invocable: true
argument-hint: "Replace <YOUR-PLAN> with your plan filename"
---
...
function Install-CursorAgent([string]$TargetPath) {
    ...
    # Generate .cursor/rules context file
    $copilotInstr = Join-Path $TargetPath ".github/copilot-instructions.md"
    ...
    if ((Test-Path $copilotInstr) -and -not (Test-Path $cursorRules)) {
        ...
        $content = Get-Content $copilotInstr -Raw
        Set-Content -Path $cursorRules -Value $content
        Write-Host "  CREATE .cursor/rules" -ForegroundColor Green
    }
