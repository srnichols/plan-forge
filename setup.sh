...
install_claude_agent() {
    ...
    if [ -f "$copilot_instr" ] && [ ! -f "$claude_ctx" ]; then
        {
            ...
            echo ""
            echo "> Auto-generated by Plan Forge from .github/copilot-instructions.md"
            echo "> Edit this file OR the source — they are independent after generation."
            echo ""
            cat "$copilot_instr"
        } > "$claude_ctx"
    ...
    # Convert pipeline prompts → Claude skills
    local prompts_dir="$target/.github/prompts"
    local skills_dir="$target/.claude/skills"
    if [ -d "$prompts_dir" ]; then
        for prompt_file in "$prompts_dir"/step*.prompt.md; do
            ...
            local desc="Plan Forge pipeline step"
            local got_desc
...

            {
                ...
                echo "argument-hint: \"Replace <YOUR-PLAN> with your plan filename\""
                echo "---"
                echo ""
                cat "$prompt_file"
            } > "$skill_file"
...
install_cursor_agent() {
    ...
    if [ -f "$copilot_instr" ] && [ ! -f "$cursor_rules" ]; then
        mkdir -p "$cursor_dir"
        cp "$copilot_instr" "$cursor_rules"
        green "  CREATE .cursor/rules"
    fi

    # Convert pipeline prompts → Cursor commands
    local prompts_dir="$target/.github/prompts"
...
install_codex_agent() {
    ...
    # Convert pipeline prompts → Codex skills
    local prompts_dir="$target/.github/prompts"
